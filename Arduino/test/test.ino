...
void loop() {
  // Delay for rat to be sprayed with pest
  delay(1000);
  // RTC
  ...
  h2 = sonar2.ping_cm();
  delay(50); // Wait 50ms between pings (about 20 pings/sec). 29ms should be the shortest delay between pings.
  //int pest_volume = 81.75 - (h2*4.5*4.5); // Calculate left pest volume
  ...
  if (buff.indexOf("Rat") >= 0) {
    send_sms("Rat sprayed: " + String(i-1));
    buff = "";
  }

  if (h1 < 7) {
    digitalWrite(fan, HIGH);
    send_sms("Rat sprayed :" + String(i));
...
void send_sms(String message) {
  sim800L.print("AT+CMGS=\"+255764797644\"\r");
  waitForResponse();
